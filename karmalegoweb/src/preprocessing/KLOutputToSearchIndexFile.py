...
from karmalegoweb.src.preprocessing.tirps import search_tirp
...
class KLOutputToSearchIndexFile:
    ...
    def __init__(self, input_file_path, output_file_path, num_of_entities, tree):

        with open(input_file_path, "r") as f:
            self._raw_data_file = f
    ...
    def parse_file(self):
        ...
        for i in range(1, len(lines), 2):
            ...
            tirp_symbols_lst = splited_line[self._TIRP_SYM_INDEX].split("-")[
                0:-1
            ]  # symbol list without empty char
            ...
            symbols = splited_line[self._TIRP_SYM_INDEX].split("-")[:-1]
            rels = splited_line[self._TIRP_REL_INDEX].split(".")[:-1]
            tirp = search_tirp(self.tree, symbols, rels)

            exists_cls0 = tirp.stats_cls0 is not None
            exists_cls1 = tirp.stats_cls1 is not None

            mean_offset_from_first_symbol_class_0 = re.sub(
                ",",
                "|",
                str(tirp.stats_cls0.mean_offset_from_first_symbol if exists_cls0 else 0),
            )
            mean_offset_from_first_symbol_class_1 = re.sub(
                ",",
                "|",
                str(tirp.stats_cls1.mean_offset_from_first_symbol if exists_cls1 else 0),
            )
            tirp_tup = (
                splited_line[self._TIRP_SYM_INDEX],
                splited_line[self._TIRP_REL_INDEX],
                tirp.size,
                exists_cls0,
                tirp.stats_cls0.vertical_support if exists_cls0 else 0,
                tirp.stats_cls0.mean_horizontal_support if exists_cls0 else 0,
                tirp.stats_cls0.mean_mean_duration if exists_cls0 else 0,
                tirp.stats_cls0.mean_duration_of_first_interval if exists_cls0 else 0,
                mean_offset_from_first_symbol_class_0 if exists_cls0 else 0,
                exists_cls1,
                tirp.stats_cls1.vertical_support if exists_cls1 else 0,
                tirp.stats_cls1.mean_horizontal_support if exists_cls1 else 0,
                tirp.stats_cls1.mean_mean_duration if exists_cls1 else 0,
                tirp.stats_cls1.mean_duration_of_first_interval if exists_cls1 else 0,
                mean_offset_from_first_symbol_class_1 if exists_cls1 else 0,
                tirp.p_value_mhs,
                tirp.p_value_md,
            )
