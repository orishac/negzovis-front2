...
class TimeIntervalsAdapter:
    ...
    def __init__(self, max_gap):
        """
        :param max_gap: int, the maximum difference of two successive timestamps in the same interval
        """
    ...
    def __connect_timestamps(self, df, minimum_duration=1):
        ...
        :return:
            numpy array of shape (nb_intervals, 2) of start times and end times,
            where nb_intervals is the number of intervals created in the process
        """
        ...
        for i, times, states in zip(count(), frames, frames_states):
            ...
            intervals[i, 1] = times[
                                  -1] + minimum_duration  # we first expand each timestamp and then connect between them
            intervals[i, 2] = states[0]

        entity_id = df[TimeIntervalsColumns.EntityID].values[0]
        prop_id = df[TimeIntervalsColumns.TemporalPropertyID].values[0]

        return pd.DataFrame({
            TimeIntervalsColumns.EntityID: entity_id,
            TimeIntervalsColumns.TemporalPropertyID: prop_id,
            TimeIntervalsColumns.Start: intervals[:, 0],
            TimeIntervalsColumns.End: intervals[:, 1],
    ...
    def create_time_intervals(self, symbolic_time_series):
        ...
        if StatesColumns.Method in symbolic_time_series.columns:
            if symbolic_time_series[StatesColumns.Method].isnull().values.any():
                ...
                time_intervals_df = symbolic_time_series \
                    .groupby(by=[StatesColumns.Method, TimeIntervalsColumns.TemporalPropertyID, TimeIntervalsColumns.EntityID]) \
                    .apply(lambda df: self.__connect_timestamps(df))
        else:
            time_intervals_df = symbolic_time_series \
                .groupby(by=[TimeIntervalsColumns.TemporalPropertyID, TimeIntervalsColumns.EntityID]) \
        ...
        time_intervals_df.reset_index(inplace=True, drop=True)
