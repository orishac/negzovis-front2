...
def candidate_selection(df, nb_bins, scoring_function, *args):
    """
    ...
    :param args: boolean parameter representing whether TD4C Entropy with information gain is being used
    ...
    if args and args[0]:
        last_entropy = -1
        entropies = np.full((len(cutpoint_candidates),), -1.0)
    ...
    for _ in range(1, nb_bins, 1):
        ...
        for i, c in enumerate(cutpoint_candidates):
            ...
            if args and args[0]:
                scores[i], entropies[i] = scoring_function(df, suggested_cutpoints, last_entropy)
            else:
        ...
        if args and args[0]:
            last_entropy = entropies[chosen_candidate_idx]
