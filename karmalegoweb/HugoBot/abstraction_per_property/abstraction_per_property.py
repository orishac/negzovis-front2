...
from constants import (
    DatasetColumns,
    PreprocessingParamsColumns,
    TemporalAbstractionParamsColumns,
    StatesColumns,
    SymbolicTimeSeriesColumns,
    get_supervised_methods_names,
    get_unsupervised_methods_names,
    MethodsNames,
)
...
from temporal_abstraction.discretization.methods.td4c_persist.td4c import (
    TD4CKullbackLeibler,
    TD4CEntropy,
    TD4CEntropyIG,
    TD4CCosine,
...
class AbstractionPerProperty:
    ...
    def preprocessing_per_property(self, dataset):
        ...
        # paa & outliers remover
        preprocessed_props = [
            reduce(
                lambda acc, new: new.transform(acc),
                AbstractionPerProperty.__generate_preprocessing_by_name(row[1]),
                dataset[
                    dataset[DatasetColumns.TemporalPropertyID]
    ...
    def temporal_abstraction_per_property(
        self, dataset, entity_class_relations=None, states=None, ncr=False
    ):
        if dataset.empty:
            return (
                DataframesGenerator.generate_empty_states(),
        ...
        symbolic_time_series = [
            AbstractionPerProperty.__generate_abstraction_by_name(
                row[1], entity_class_relations, states, ncr
            ).discretize_property(
                dataset[
                    dataset[DatasetColumns.TemporalPropertyID]
                    == row[1][TemporalAbstractionParamsColumns.TemporalPropertyID]
                ]
            )
        ...
        state_ids_ctr = reduce(
            lambda acc, new: acc + [acc[-1] + new[0].shape[0]], symbolic_time_series, [0]
        )
        ...
        def reindex_states(tmp_states, states_ctr):
            return {
                StatesColumns.StateID: tmp_states[StatesColumns.StateID]
                + states_ctr
        ...
        symbolic_time_series = [
            ...
            for ((tmp_states, sts), states_ctr) in zip(symbolic_time_series, state_ids_ctr)
        ]

        """ Concatenating the data-frames one after the other """
        """ Will Sort if columns do not match which leads to non-concatenation axis is not aligned """
    ...
    def __generate_preprocessing_by_name(params_row):
        ...
        if np.isnan(params_row[PreprocessingParamsColumns.PAAWindowSize]):
            params_row[PreprocessingParamsColumns.PAAWindowSize] = 1
        res.append(PAA(params_row[PreprocessingParamsColumns.PAAWindowSize]))
        if np.isnan(params_row[PreprocessingParamsColumns.StdCoef]):
            params_row[PreprocessingParamsColumns.StdCoef] = -1
        res.append(OutliersRemover(params_row[PreprocessingParamsColumns.StdCoef]))

        return res

    @staticmethod
    def __generate_abstraction_by_name(
        params_row, entity_class_relations=None, states=None, ncr=False
    ):
        """
        ...
        if name in supervised_methods:
            ...
            get_params = lambda params: [int(params[TemporalAbstractionParamsColumns.NbBins])]
        elif name == MethodsNames.KnowledgeBased:
            get_params = lambda params: [states, ncr]
        elif name == MethodsNames.Gradient:
            get_params = lambda params: [
                int(params[TemporalAbstractionParamsColumns.GradientWindowSize]),
                states,
                ncr,
            ]
        else:
