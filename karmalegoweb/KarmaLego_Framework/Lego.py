...
from karmalegoweb.KarmaLego_Framework.LegoTree import LegoTreeNode
from karmalegoweb.KarmaLego_Framework.StiInstance import StiInstance
from karmalegoweb.KarmaLego_Framework.TIRP import TIRP
...
class Lego(object):
    ...
    def fit(self, index_same=False, skip_followers=False, processes_num=None):
        ...
        :return: None
        """
        with ThreadPoolExecutor(processes_num) as executor:
            nodes_processes = []
            index = 0
            for symbol1_id, symbol1 in self._karma.get_symbols_map().items():
                index += 1
                if (
                    self._karma.get_symbol_id_vertical_support(symbol1_id)
                ...
                if self.need_one_sized:
                    ...
                    if self.incremental_output:
                        one_sized_tirp.print_tirp(
                            self.path,
                            self._karma.get_entities_vector(),
                            self._karma._num_relations,
                            # Karma.CALC_OFFSETS,
                            True,
                ...
                for symbol2_id, symbol2 in self._karma.get_symbols_map().items():
                    for rel in range(self._karma._num_relations):
                        checked_vs = self._karma.get_vertical_support_of_sym_to_sym_rel(
                                symbol1.symbol_index, symbol2.symbol_index, rel
                            )
                        if (
                            checked_vs == 0 or 
                            (checked_vs > 0 and checked_vs < self._karma.get_min_vertical_support())
                        ):
                            continue
                        tirp = self._karma.get_two_sized_tirp_for_symbols_and_rel(
                            symbol1_id, symbol2_id, rel
                        )
                        if self.incremental_output:
                            tirp.print_tirp(
                                self.path,
                        ...
                        nodes_processes.append(
                            executor.submit(self.extend_tirp, tirp, index_same, skip_followers)
                        )
            #     root_node.add_node(node)
            for job in nodes_processes:
                job.result()
            # for node in nodes_processes:
            # self.insert_frequent_tirps(root_node)
        current_app.logger.info("Lego fit complete")
    ...
    def add_instances_to_one_sized_tirp(self, tirp):
        ...
        for entity_index, instances in sup_insts.items():
            for inst in instances:
                ...
                tirp._supporting_sequences_by_entity[
                    self._karma.get_entities_vector()[entity_index]
                ].append(instance)
    ...
    def extend_tirp(self, tirp, index_same, skip_followers):
        ...
        for symbol_id, symbol in self._karma.get_symbols_map().items():
            ...
            for seed_relation in range(self._karma._num_relations):
                if (
                    self._karma.get_vertical_support_of_sym_to_sym_rel(
                        self._karma.get_symbol_by_id(tirp._symbols[tirp.size - 1]).symbol_index,
                        symbol.symbol_index,
                        seed_relation,
                    )
                    >= self._karma.get_min_vertical_support()
                    ...
                    for candidate_relations in candidates:
                        new_tirp = self.extend_single_tirp(
                            tirp, symbol_id, candidate_relations, skip_followers
                        )
                        if (
                            len(new_tirp.supporting_entities)
                            >= self._karma.get_min_vertical_support()
                        ):
                            new_tirp._support_discovery = round(
                                len(new_tirp._supporting_sequences_by_entity)
                                / len(self._karma._entities),
                                2,
                            )
                            if self.incremental_output:
                                new_tirp.print_tirp(
        ...
        tirp_file_name = root_node.tirp.get_tirp_file_name(self._karma._num_relations)
        root_node.tirp.print_tirp(
            self.path,
            self._karma.get_entities_vector(),
            self._karma._num_relations,
            # Karma.CALC_OFFSETS,
    ...
    def generate_candidates(self, tirp, seed_relation):
        ...
        for rel_index_to_set in rng:
            left_tirp_index = int(
                ((rel_index_to_set + 1) * rel_index_to_set / 2) + rel_index_to_set
            )
            ...
            for cand_index in range(cand_list_size):
                ...
                transitivity_list = self._karma._relation_handler_obj.get_transitivity_list(
                    tirp._tirp_matrix.get_relations()[left_tirp_index], candidate[below_cnd_index]
                )
    ...
    def generate_candidates_previous(self, tirp, relation):
        ...
        candidates = self.generate_candidates_helper(
            tirp._tirp_matrix.get_all_direct_relations(), relation, len(tirp._symbols) - 2
        )
    ...
    def generate_candidates_helper(self, tirp_last_relation_column, first_relation, index):
        ...
        if index < 0:
            ...
            return ans
        transitivity_list = self._karma._relation_handler_obj.get_transitivity_list(
            tirp_last_relation_column[index], first_relation
        )
        for relation in transitivity_list:
            results = self.generate_candidates_helper(
                tirp_last_relation_column, relation, (index - 1)
            )
            for res in results:
                res.append(relation)
                ans.append(res)
    ...
    def extend_single_tirp(self, tirp, new_symbol, new_relations, skip_followers):
        ...
        for i in range(len(tirp.supporting_instances)):
            tis_list = (
                self._karma.get_stis_in_relation_with_key_sti_for_entity_by_relation_and_symbols(
                    sym_last_index,
                    sym_new_index,
                    seed_relation,
                    tirp.supporting_instances[i].entity_index,
            ...
            for tis in tis_list:
                ...
                for rel_index in range(top_rel_index, seed_rel_index):
                    ...
                    if (
                        skip_followers
                        and abs(float(tis._symbol) - float(second_tis._symbol)) == 1
                        and tis._var_id == second_tis._var_id
                    ):
                        did_break = True
                        break
                    found_rel = self._karma._relation_handler_obj.check_relation(
                        second_tis, tis, self._karma.get_epsilon(), self._karma.get_max_gap()
                    )
                ...
                if not did_break:
                    ...
                    new_instance = self.extend_instance(
                        new_tirp.size,
                        tirp.supporting_instances[i].sti,
                        tis,
                        entity_index,
                        tirp.supporting_instances[i].start,
                        tirp.supporting_instances[i].end,
                    )
                    new_tirp.add_entity(
                        entity_index, self._karma.get_entities_vector()[entity_index]
                    )
                    new_tirp.supporting_instances.append(new_instance)
                    new_tirp._supporting_sequences_by_entity[
                        self._karma.get_entities_vector()[entity_index]
                    ].append(new_instance)
    ...
    def extend_instance(self, size, instance, last, entity_index, start, end):
        return StiInstance.extend_instance(
            size,
            instance,
            last,
            entity_index,
