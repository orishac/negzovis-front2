...
from karmalegoweb.KarmaLego_Framework.Symbol import Symbol
from karmalegoweb.KarmaLego_Framework.TIRP import TIRP
from karmalegoweb.KarmaLego_Framework.Tindex import Tindex
from karmalegoweb.KarmaLego_Framework.StiInstance import StiInstance
from karmalegoweb.KarmaLego_Framework.utils.parse import *
...
class Karma(object):
    ...
    def fit(
        ...
        :param entity_ids_num: int, #Numbers in the entity id lines of the file
    ...
    def parse_kml_input(self, file_path, num_comma, symbol_type, entity_ids_num, semicolon_end):
        ...
        self.parse_file_into_symbolic_time_intervals_parts(
            file_path,
            self._symbolic_interval_start_point,
            symbol_type,
            num_comma,
            entity_ids_num,
            semicolon_end,
    ...
    def parse_file_into_symbolic_time_intervals_parts(
        self, file_path, start_index, symbol_type, num_comma, entity_ids_num, semicolon_end
    ):
        """
        ...
        for i in range(0, len(lines), 2):
            ...
            for sym_ti in intervals_list:
                sym_ti_obj, symbol = parser_of_sym_ti_row_for_different_formats(
                    sym_ti, num_comma, symbol_type
                )
                if len(self.selected_variables) == 0:
                    ...
                    self._symbols_map[symbol].add_interval_to_entity(
                        len(self._entities_map) - 1, sym_ti_obj
                    )
        ...
        if symbol_type == "int":
            ...
            for entity_id in self._entities_map:
                self._entities_map[entity_id].sort(
                    key=operator.attrgetter("start_time", "end_time", "symbol")
                )
    ...
    def get_vertical_support_of_sym_to_sym_rel(self, sym1_index, sym2_index, rel):
        return len(
            self._karma_index.get_vertical_support_of_symbol_to_symbol_relation(
                sym1_index, sym2_index, rel
            )
        )
    ...
    def get_stis_in_relation_with_key_sti_for_entity_by_relation_and_symbols(
        self, first_index, second_index, rel, entity_index, tis
    ):
        return (
            self._karma_index.get_stis_in_relation_with_key_sti_for_entity_by_relation_and_symbols(
                first_index, second_index, rel, entity_index, tis
            )
        )
    ...
    def build_karma_index(self, skip_followers, index_same, filter_overlapping_grad_state):
        ...
        self._karma_index.build_tindex(
            len(self._symbols_map), len(self._entities_map), self._num_relations, True
        )
        for entity_index in range(len(self._entities)):
            ...
            for ti1_index in range(len(tis_list)):
                ...
                for ti2_index in range(ti1_index + 1, len(tis_list)):
                    ...
                    if (
                        skip_followers
                        and abs(float(first_tis._symbol) - float(second_tis._symbol)) == 1
                        and first_tis._var_id == second_tis._var_id
                    ):
                        continue
                    if (
                        filter_overlapping_grad_state
                        and first_tis._var_id is not None
                        and first_tis._var_id == second_tis._var_id
                        and first_tis._end_time >= second_tis._start_time
                    ):
                        continue
                    self.add_symbol(entity_index, second_tis)
                    relation = self._relation_handler_obj.check_relation(
                        first_tis, second_tis, self._epsilon, self._max_gap
                    )
                    if relation != RelationHandler.RELATION_NOT_DEFINED:
                        ...
                        self._karma_index.add_sti_to_tindex_relation_entry_by_symbols(
                            index1, index2, relation, entity_index, first_tis, second_tis
                        )
                    else:
                        break
    ...
    def get_two_sized_tirp_for_symbols_and_rel(self, first_id, second_id, rel):
        ...
        ti_list_dics_vec = self._karma_index.kindex[index1][
            index2
        ].get_sti_maps_in_array_indexed_by_entity_id_for_relation(rel)
        entities = (
            ti_list_dics_vec.keys()
            if type(ti_list_dics_vec) is dict
        ...
        for entity_index in entities:
            ...
            for ti_list_ti_key, ti_list_ti_value in ti_list_ti_dic.items():
                for tis in ti_list_ti_value:
                    sti = SymbolicTimeInterval(
                        ti_list_ti_key._start_time,
                        ti_list_ti_key._end_time,
                        ti_list_ti_key._symbol,
                        ti_list_ti_key._var_id,
                    )
                    ...
                    two_sized_tirp._supporting_sequences_by_entity[
