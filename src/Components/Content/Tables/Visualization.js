...
class Visualization extends Component {
	constructor(props) {
		...
		this.negative_data = this.getVisualizationNegativeData();
		this.state = {
			kl_id: null,
		};
	}

	getVisualizationNegativeData() {
		return this.props.NegativesTable.filter(
		...
		return (
			<div>
				...
				<Card style={{ width: 'auto' }}>
					<Card.Header className={'bg-hugobot'}>
						<Card.Text className={'text-hugobot text-hugoob-advanced'}>
							Negative Datasets ready for visualization
						</Card.Text>
					</Card.Header>
					<Card.Body className={'text-hugobot'}>
						<div className='vertical-scroll-tirp'>
							<Table striped={true} bordered={true} hover={true}>
								...
								<tbody>
									{this.negative_data.map((visualization, index) => {
										const selected = this.state.kl_id === visualization.kl_id;
										return (
											<tr
												key={index}
