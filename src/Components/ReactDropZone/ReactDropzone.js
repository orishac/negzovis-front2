...
export class DropzoneComponent extends React.Component {
	...
	render() {
		...
		const className = this.props.className ? 'dropzone ' + this.props.className : 'dropzone';
	...
	setupEvents() {
		...
		for (var eventHandler in eventHandlers) {
			if (eventHandlers.hasOwnProperty(eventHandler) && eventHandlers[eventHandler]) {
				...
				if (
					Object.prototype.toString.call(eventHandlers[eventHandler]) === '[object Array]'
				) {
